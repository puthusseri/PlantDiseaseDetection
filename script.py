...
def analysis(filepath):
	...
	if np.argmax(model_out) == 0:
		...
		str_label = 'Bacterial'
		rem = " Discard or destroy any affected plants. Do not compost them. Rotate yoour tomato plants yearly to prevent re-infection next year. Use copper fungicites"
	elif np.argmax(model_out) == 2:
		str_label = 'Viral'
		rem = " Monitor the field, handpick diseased plants and bury them. Use sticky yellow plastic traps. Spray insecticides such as organophosphates, carbametes during the seedliing stage. Use copper fungicites"
	elif np.argmax(model_out) == 3:
		str_label = 'Lateblight'
		rem = " Monitor the field, remove and destroy infected leaves.  Treat organically with copper spray.  Use chemical fungicides,the best of which for tomatoes is chlorothalonil."
